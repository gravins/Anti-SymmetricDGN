...
#@ray.remote(num_cpus=os.getenv('OMP_NUM_THREADS',1))
@ray.remote(num_cpus=2, num_gpus=1/10)
def train_and_eval(model_class, config, 
    ...
    # Load dataset
    data, _, _ = get_dataset(root=data_dir, name=data_name)
    ...
    for i in range(10):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=config['optim']['lr'], weight_decay=config['optim']['wd'])
        max_score = -1
        best_epoch = 0
        best_score = None
        ...
        model.load_state_dict(ckpt['model_state_dict'])
        model.to(device)
        test_scores, _ = evaluate(model, data, test_mask, criterion)
